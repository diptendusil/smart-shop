...
public class ProductController {
	...
	public List<ProductDto> getRecommendations(@PathVariable String userId) {
		...
		JsonNode obj = restTemplate.getForObject("http://billing-service/bill/all", JsonNode.class);
		List<BillDTO> bills = mapper.convertValue(obj, new TypeReference<List<BillDTO>>(){});
		return convertProductsToProductDtos(this.productService.getRecommendations(bills, userId, this.categoryService.getAllCategories()));
	}
