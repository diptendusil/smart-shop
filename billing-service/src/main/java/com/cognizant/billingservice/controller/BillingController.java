...
public class BillingController {
	...
	public List<BillDTO> getAllBillsByUser(@PathVariable("username") String username) {
		...
		return this.billService.getBillsByUser(user).stream().map(bill -> convertBillToBillDTO(bill)).collect(Collectors.toList());
	}
	@PostMapping
	...
	public PurchaseItem convertPurchaseItemDTOToPurchaseItem(PurchaseItemDTO purchaseItemDTO) {
		...
		PurchaseItem purchaseItem = new PurchaseItem(purchaseItemDTO.getPurchaseId(),purchaseItemDTO.getQuantity(), purchaseItemDTO.getPrice(), product, null);
		return purchaseItem;
	}
	public PurchaseItemDTO convertPurchaseItemToPurchaseItemDTO(PurchaseItem purchaseItem) {
		...
		PurchaseItemDTO purchaseItemDTO = new PurchaseItemDTO(purchaseItem.getPurchaseId(),purchaseItem.getQuantity(), purchaseItem.getPrice(), productDTO, null);
		return purchaseItemDTO;
	}
	public Bill convertBillDTOToBill(BillDTO billDTO) {
		List<PurchaseItem> purchaseItems = billDTO.getPurchaseItems().stream().map(purchaseItemDTO -> convertPurchaseItemDTOToPurchaseItem(purchaseItemDTO)).collect(Collectors.toList());
		Bill bill = new Bill(billDTO.getBillId(), billDTO.getTotal(), billDTO.getRewardPoints(), billDTO.getDate(),billDTO.getUser(), purchaseItems);
		return bill;
	}
	public BillDTO convertBillToBillDTO(Bill bill) {
		List<PurchaseItemDTO> purchaseItems = bill.getPurchaseItems().stream().map(purchaseItem -> convertPurchaseItemToPurchaseItemDTO(purchaseItem)).collect(Collectors.toList());
		BillDTO billDTO = new BillDTO(bill.getBillId(), bill.getTotal(), bill.getRewardPoints(), bill.getDate(), bill.getUser(), purchaseItems);
		return billDTO;
	}
}
